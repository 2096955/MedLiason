...
export default function CompletionStep({
  ...
  const getBrokerTypeText = (type: string) => {
    switch (type) {
      case "solace":
        return "Existing MedExpert Pub/Sub+ broker";
      case "container":
        return "New local MedExpert PubSub+ broker container (podman/docker)";
      case "dev_mode":
  ...
  const renderGroup = (groupName: string, keys: string[]) => {
    ...
    return (
      ...
      >
        <h4 className="font-semibold text-medexpert-blue mb-3">{groupName}</h4>
