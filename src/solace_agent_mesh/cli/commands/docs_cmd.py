...
class DocsHttpRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith('/medexpert'):
            self.path = self.path[len('/medexpert'):] or '/'
        super().do_GET()

    def send_error(self, code, message=None):
        if code == 404:
            self.send_response(302)
            self.send_header('Location', '/medexpert/docs/documentation/getting-started/introduction/')
            self.end_headers()
        else:
...
def docs(port: int):
    ...
    with socketserver.TCPServer(("", port), handler) as httpd:
        url = f"http://localhost:{port}/medexpert/docs/documentation/getting-started/introduction/"
