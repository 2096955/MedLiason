...
def broker_setup_step(
    ...
    broker_type_prompt = (
        ...
        "  1) Existing MedExpert Pub/Sub+ broker\n"
        "  2) New local MedExpert PubSub+ broker container (requires Podman or Docker)\n"
    ...
    if broker_type in ["2", "container"]:
        ...
        if not has_podman and not has_docker:
            click.echo(
                click.style(
                    "Error: Podman or Docker is required to run a local MedExpert broker container.",
                    fg="red",
                ),
                err=True,
            )
        ...
        click.echo(
            f"Attempting to run MedExpert PubSub+ broker container using {container_engine}:"
        )
        ...
        if not skip_interactive:
            if not click.confirm(
                ...
                options["broker_url"] = ask_if_not_provided(
                    options,
                    "broker_url",
                    "Enter MedExpert broker URL endpoint",
                ...
                options["broker_vpn"] = ask_if_not_provided(
                    options,
                    "broker_vpn",
                    "Enter MedExpert broker VPN name",
                ...
                options["broker_username"] = ask_if_not_provided(
                    options,
                    "broker_username",
                    "Enter MedExpert broker username",
                ...
                options["broker_password"] = ask_if_not_provided(
                    options,
                    "broker_password",
                    "Enter MedExpert broker password",
        ...
        try:
            ...
            if response_status != 0:
                click.echo(
                    click.style(
                        f"Error: Failed to start MedExpert PubSub+ broker container (exit code: {response_status}). Check container logs.",
                        fg="red",
                    ),
                    err=True,
                )
            ...
            click.echo(
                click.style(
                    "MedExpert PubSub+ broker container started successfully (or already running).",
                    fg="green",
                )
            )
