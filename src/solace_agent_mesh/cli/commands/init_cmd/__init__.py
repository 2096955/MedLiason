...
DEFAULT_INIT_VALUES = {
    ...
    "MEDEXPERT_LOCAL_BROKER_URL": "ws://localhost:8008",
    "MEDEXPERT_LOCAL_BROKER_VPN": "default",
    "MEDEXPERT_LOCAL_BROKER_USERNAME": "default",
    "MEDEXPERT_LOCAL_BROKER_PASSWORD": "default",
...
def run_init_flow(skip_interactive: bool, use_web_based_init_flag: bool, **cli_options):
    """
    Orchestrates the initialization of a new MedExpert application project
    by running a sequence of steps.
    """
    click.echo(
        click.style("Initializing MedExpert Application Project...", bold=True, fg="blue")
    )
    ...
    click.echo(
        click.style(
            "Next steps: Consider running 'sam run'.",
            fg="blue",
        )
    )
...
@click.option(
    ...
    help="Broker type: 1/existing, 2/container (new local), 3/dev (dev mode).",
)
@click.option("--broker-url", type=str, help="MedExpert broker URL endpoint.")
@click.option("--broker-vpn", type=str, help="MedExpert broker VPN name.")
@click.option("--broker-username", type=str, help="MedExpert broker username.")
@click.option("--broker-password", type=str, help="MedExpert broker password.")
@click.option(
    "--container-engine",
...
def init(**kwargs):
    """
    Initialize a new MedExpert application project.
