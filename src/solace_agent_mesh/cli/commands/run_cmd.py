...
def _execute_with_solace_ai_connector(config_file_paths: list[str]):
    ...
    sys.argv = [
        sys.argv[0].replace("medexpert", "solace-ai-connector"),
        *config_file_paths,
    ]
...
def run(files: tuple[str, ...], skip_files: tuple[str, ...], system_env: bool):
    """
    Run the MedExpert application with specified or discovered YAML configuration files.
