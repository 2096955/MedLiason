#!/usr/bin/env bash
# Multi-model MedExpert startup for Cloud Run single-container deployment.
#
...
for cfg in "${REQUIRED_CONFIGS[@]}"; do
  if [ ! -s "$cfg" ]; then
    echo "[MultiModel] FATAL: Required config not found or empty: $cfg"
    exit 1
  fi
done

echo "[MultiModel] Starting multi-model MedExpert (39 agents, 3 stacks)..."
echo "[MultiModel] Host: 0.0.0.0, Port: ${PORT:-8080}"
echo "[MultiModel] LiteLLM: $(python -c 'from litellm._version import version; print(version)' 2>/dev/null || echo 'unknown')"
...
# ---------------------------------------------------------------------------
echo "[MultiModel] Starting Redis..."
...
if ! redis-cli ping 2>/dev/null | grep -q PONG; then
  echo "[MultiModel] FATAL: Redis failed to start"
  exit 1
fi
echo "[MultiModel] Redis ready"
...
for entry in "${MCP_SERVERS[@]}"; do
  module="${entry%%:*}"
  port="${entry##*:}"
  echo "[MultiModel] Starting MCP: $module (port $port)"
  python -m "$module" &
  MCP_PIDS+=($!)
done
...
# ---------------------------------------------------------------------------
echo "[MultiModel] Waiting for MCP servers..."
for entry in "${MCP_SERVERS[@]}"; do
  port="${entry##*:}"
  module="${entry%%:*}"
  ready=false
  ...
  if [ "$ready" = true ]; then
    echo "[MultiModel]   Port $port ready ($module)"
  else
    echo "[MultiModel]   WARNING: Port $port not ready ($module) — continuing (graceful degradation)"
  fi
done
...
# ---------------------------------------------------------------------------
echo "[MultiModel] Starting Memgraph MCP server..."
...
if [ "$ready" = true ]; then
  echo "[MultiModel]   Port 8000 ready (memgraph-mcp)"
else
  echo "[MultiModel]   WARNING: Port 8000 not ready (memgraph-mcp) — continuing (graceful degradation)"
fi
...
cleanup() {
  ...
  echo "[MultiModel] Shutdown complete"
}
...
# ---------------------------------------------------------------------------
echo "[MultiModel] Starting sam run with ${#REQUIRED_CONFIGS[@]} configs (39 agents + 1 gateway)..."
